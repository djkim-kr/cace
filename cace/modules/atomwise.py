...
class Atomwise(nn.Module):
    ...
    def __init__(
        ...
        charge_key: Optional[str] = None,  # for multi-head output
        ...
        if charge_key is not None:
            self.model_outputs.append(charge_key)
    ...
    def forward(self, 
        ...
        if self.charge_state is not None:
            charge_state = torch.full((features.shape[0], 1), self.charge_state, device=features.device, dtype=features.dtype)
